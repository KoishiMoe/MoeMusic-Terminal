...
internal class StandaloneCoverArtRenderer(
    ...
    private sealed interface CoverState {
        ...
        data class Ready(val cells: List<List<TextCharacter>>) : CoverState
    }
    ...
    fun draw(
        ...
        when (val state = stateFor(key)) {
            ...
            is CoverState.Ready -> drawCells(graphics, x, y, width, height, state.cells)
        }
    }
    ...
    private fun loadAsync(key: String, url: String, width: Int, height: Int) {
        scope.launch {
            val state = runCatching {
                ...
                CoverState.Ready(renderCells(source, width, height))
            }.getOrElse {
                CoverState.Failed
            }
            remember(key, state)
    ...
    private fun coverLimits(width: Int, height: Int): CoverLimits {
        ...
        return CoverLimits(
            ...
            decodeTargetSize = minOf(config.maxTextureSize, maxOf(width, height * 2).coerceAtLeast(16)),
        )
    }
